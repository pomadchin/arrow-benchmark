...
class ArrowVectorByteBenchmark {
  /**
  ...
  private var arrowVector: UInt1Vector = _
  ...
  private def createArrow(buf: ByteBuffer): Unit = {
    arrowVector = new UInt1Vector("arrowVector", alloc)
    arrowVector.allocateNew(buf.capacity)
    while (buf.hasRemaining) arrowVector.set(buf.position, buf.get)
    arrowVector.setValueCount(buf.capacity)
  }

  private def createBuffer(buf: ByteBuffer): Unit = buffer = buf
